...
export class SignupPage {
  ...
  async handlePDSResult(allowRecoveryLinkClick = false) {
    ...
    const mismatchLink = this.page
      .locator(
        [
          'span[role="button"]:has-text("I wish to use a different email and/or phone number for my appointment")',
          'a:has-text("I wish to use a different email and/or phone number for my appointment")',
          'button:has-text("I wish to use a different email and/or phone number for my appointment")',
          'span:has-text("I wish to use a different email and/or phone number for my appointment")',
    ...
    if (await privateLink.isVisible().catch(() => false)) {
      ...
      if (shouldClickRecovery) {
        console.log(
          "[SignupPage] Clicking recovery contact-details link to open modal",
        );
    ...
    let isModalOpen = await this.page
      .locator(
  ...
  async handleMandatoryCheckboxes() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!isChecked) {
        console.log(
          `[SignupPage] Checking mandatory checkbox: "${(text || "").trim().slice(0, 50)}..."`,
        );
  ...
  async fillContactDetails(
    ...
    console.log(
      `[SignupPage] Phone fields filled -> phone=${phoneFilled}, confirmPhone=${confirmPhoneFilled}`,
    );
  ...
  async submitAndBook(allowRecoveryLinkClick = false) {
    ...
    if (recoveryModalStillOpen) {
      ...
      if (await modalConfirm.isVisible().catch(() => false)) {
        console.log(
          "[SignupPage] Recovery modal still open — retrying popup Confirm",
        );
